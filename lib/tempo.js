...
const getWorklogs = async (accountKey, dateFrom, dateTo) => {
  ...
  if (TEMPO_STRATEGY === 'system') {
    ...
    let worklogs = []
    if (res.worklogs.worklog.hasOwnProperty('worklog_id')) {
      worklogs = [res.worklogs.worklog]
    } else if (res.worklogs.worklog.map) {
      worklogs = res.worklogs.worklog
    }

    console.log({ count: worklogs.length })
    const results = worklogs.map(r => ({
      ...r,
